...
public class MediaScreenEncoder extends MediaVideoEncoderBase {
    ...
    private final int bitrate, fps, orientation;
    ...
    public MediaScreenEncoder(final MediaMuxerWrapper muxer, final MediaEncoderListener listener,
                              ...
                              final int _bitrate, final int _fps, final int _orientation) {

        super(muxer, listener, width, height);
        mMediaProjection = projection;
        mDensity = density;
        fps = (_fps > 0 && _fps <= 60) ? _fps : FRAME_RATE;
    ...
    void prepare() throws IOException {
        ...
        mSurface = prepare_surface_encoder(MIME_TYPE, fps, bitrate, orientation);
        mMediaCodec.start();
        mIsRecording = true;
